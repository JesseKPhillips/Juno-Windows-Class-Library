...
int CoGetClassObject(inout GUID rclsid, uint dwClsContext, void* pvReserved, inout GUID riid, void** ppv);
...
template DerivesFrom(T, TList ...) {
  const bool DerivesFrom = is(std.typetuple.MostDerived!(T, TList) : T);
}
 ...
 * If any of the type parameters derive from IDispatch, the class provides a default implementation of IDispatch.
 * Examples:
 * ---
 * class XmlDocumentEvents : Implements!(XMLDOMDocumentEvents) {
 * }
 *
 * XmlDocumentEvents docEvents = new XmlDocumentEvents;
 * docEvents.Invoke(...);
 * ---
 */
  ...
  public T com_cast_impl(U)(U obj) {
    static if (is(U : IUnknown)) {
      ...
      static if (vt != VT_VOID) {
        VARIANT v;
        if (VariantChangeTypeEx(&v, &obj, GetThreadLocale(), 0, vt) == S_OK) {
        ...
        static assert(false, "Cannot cast from '" ~ juno.base.meta.nameof!(U) ~ "' to '" ~ juno.base.meta.nameof!(T) ~ "'.");
    }
    else
      static assert(false, "Cannot cast from '" ~ juno.base.meta.nameof!(U) ~ "' to '" ~ juno.base.meta.nameof!(T) ~ "'.");
  }
...
/**
 * Deduces the COM type of another type.
 */
...
struct Iteration(T, U) {
  ...
  invariant {
    static if (is(T == SAFEARRAY*)) {
      ushort vt;
      SafeArrayGetVartype(source, vt);
      assert(vt == VariantType!(U), "Specified safe array was not of the expected type.");
    }
  }

  static Iteration opCall(T source) {
    Iteration it;
    return it.source = source,
      it;
  }

  int opApply(int delegate(inout U) action) {
    int r = 0;
    ...
    static if (is(T == SAFEARRAY*)) {
      ...
      int index = 0;
      while (index < end) {
        hr = SafeArrayGetElement(source, &index, &u);
        if (hr != S_OK || (r = action(u)) != 0)
          break;
        index++;
      }
    }
      ...
      uint n;
      while ((hr = source.Next(1, &u, n)) == S_OK) {
        if (n == 0 || (r = action(u)) != 0)
          break;
      }
      source.Reset();
  ...
  int opApply(int delegate(inout int, inout U) action) {
    int r = 0;
...
template iterateOver(T) {
  Iteration!(S, T) iterateOver(S)(S it) {
    return Iteration!(S, T)(it);
  }
}