...
public class IndentingWriter : Writer {
  ...
  public override void newLine(string value) {
...
public class CodeGenerator {
  ...
  public void generateCodeFromTypeLibrary(TypeLibrary typeLib, Writer w) {
    ...
    if (emitComments) {
      foreach (reference; typeLib.getReferences()) {
        if (reference !is null) {
          string fileName = getFileName(reference.location);
          if (fileName != null)
  ...
  private void generateMethod(Method method, Type declaringType) {
    if (currentType is null || (currentType.isInterface || currentType.isCoClass || currentType.isStruct)) {
      ...
      if (emitComments) {
        ...
        output.write("/*[id(0x{0:X8})]*/", method.id);
        output.write(" ");
      }
  ...
  private void generateVariable(VARIANT var) {
    ...
    string value;
    if (var.vt == VARTYPE.VT_I4)
      value = format("0x{0:X8}", var.lVal);
    else if (var.vt == VARTYPE.VT_BOOL)
      value = (var.boolVal == VARIANT_TRUE) ? "true" : "false";
    else if (var.vt == VARTYPE.VT_NULL)
      value = "null";
    else
      value = var.toString();
    if (var.vt == VARTYPE.VT_BSTR || var.vt == VARTYPE.VT_LPSTR || var.vt == VARTYPE.VT_LPWSTR)
